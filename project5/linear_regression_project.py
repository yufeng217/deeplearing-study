...
#1、参数星号的作用是什么，造成了行列转换是结果，但是要回答为什么*和zip组合为什么可以造成行列转换这个结果
#     *M 中 * 的作用是将 M(不定长参数)解压为一个一个的元组，zip 将 *M 中对应的元素打包成一个个元组，然后返回由这些元组组成的对象。从下面的例子能看出效果。
#2、col不是int类型，所以回答int是错误的，请修正col的类型答案哦~（type(变量)方法 ，print一下就知道了）
#     经过实际代码输出，确定 col 是 tuple 类型
#3、另外请补充说明最后为什么要用list()呢？
#     用 list 是要把元组转换成数组，这样才能满足矩阵转置的需要。
    ...
    if shape_a[1] != shape_b[0]:
        ...
        for i in range(shape_a[0]):
            ...
            for j in range(shape_b[1]):
                #列数
                temp.append(sum([a * b for a, b in zip(A[i], transpose(B)[j])]))
            result.append(temp)   
        #print(shape(result))
        #print(result)
    return result
...
def is_near_0(n, epsilon=1.0e-16):
    return abs(n - 0.0) <= epsilon

def is_near_1(n, epsilon=1.0e-16):
    return abs(n - 1.0) <= epsilon

# TODO r1 <--- r1 * scale
# scale为0是非法输入，要求 raise ValueError
# 直接修改参数矩阵，无返回值
def scaleRow(M, r, scale, epsilon=1.0e-16):
    #print("row : {}, scale : {}".format(r, scale))
    if scale == 0:
        raise ValueError
    else:
        M[r] = [1.0 if is_near_1(x * scale, epsilon) else x * scale for x in M[r]]
...
def addScaledRow(M, r1, r2, scale, epsilon=1.0e-16):
    M[r1] = [0.0 if is_near_0(rx + rys) else rx + rys for rx, rys in zip(M[r1], [ry*scale for ry in M[r2]])]
...
def gj_Solve(A, b, decPts=4, epsilon=1.0e-16):
    ...
    for i in range(len(Ab)):
        col = transpose(Ab)[i][i:]
        #print(col)
        col_max = max(map(abs, col))
        if col_max in col:
            ri_max = col.index(col_max) + i
        else:
            ri_max = col.index(-col_max) + i
        if is_near_0(col_max, epsilon):
            col_max = 0.0
        if col_max == 0.0:
            # A为奇异矩阵
            return None
            ...
            # 使用第一个行变换，将绝对值最大值所在行交换到对角线元素所在行（行c）
            swapRows(Ab, i, ri_max)
            # 使用第二个行变换，将列c的对角线元素缩放为1
            scaleRow(Ab, i, 1./Ab[i][i], epsilon)
            # 多次使用第三个行变换，将列c的其他元素消为0
            # 由于被交换过位置，重新获取当前列进行循环
            current_col = transpose(Ab)[i]
            for c_i, c_x in enumerate(current_col):
                if c_i != i:
                    addScaledRow(Ab, c_i, i, -c_x, epsilon)
...
def calculateMSE2D(X,Y,m,b):
    return sum([(yi - m*xi - b)**2 for xi, yi in zip(X, Y)])/ len(X)
